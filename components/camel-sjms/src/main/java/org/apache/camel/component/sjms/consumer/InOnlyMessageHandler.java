...
public class InOnlyMessageHandler extends DefaultMessageHandler {
     ...
     * @param processor
     */
    public InOnlyMessageHandler(Endpoint endpoint, AtomicBoolean stopped, ExecutorService executor) {
        ...
        if (isStarted()) {
            if (exchange.isFailed()) {
                ...
                if (log.isTraceEnabled()) {
                    log.trace("Processing Exchange id:{} synchronously", exchange.getExchangeId());
                }
                try {
            ...
            log.warn("SjmsMessageConsumer invoked while stopped.  Exchange id:{} will not be processed.", exchange.getExchangeId());
        }
